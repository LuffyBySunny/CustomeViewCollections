...
public class TabLayout extends HorizontalScrollView {
     ...
     * 默认值，不设置粗体
     */
    public static final int BOLD_NORMAL = 0;

    /**
     * 选中的Tab文字粗体
     */
    public static final int BOLD_SELECTED = 1;

    /**
     * 所以的Tab字体粗体
     */
    public static final int BOLD_ALL = 2;
    ...
    public TabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (a.hasValue(R.styleable.TabLayout_tabSplitterWidth)) {
            mTabStrip.setSplitterWidth(a.getDimensionPixelSize(R.styleable.TabLayout_tabSplitterWidth, 0));
            mTabStrip.setSplitterColor(a.getColor(R.styleable.TabLayout_tabSplitterColor, 0));
            mTabStrip.setSplitterPadding(a.getDimensionPixelSize(R.styleable.TabLayout_tabSplitterPadding, 0));
        }
        mTabStrip.setIndicatorVisibility(a.getInt(R.styleable.TabLayout_tabIndicatorVisibility, VISIBLE));
        ...
        mTabBold = a.getInt(R.styleable.TabLayout_tabBold, BOLD_NORMAL);
        a.recycle();
    ...
    class TabView extends LinearLayout {
        ...
        public void setSelected(final boolean selected) {
            ...
            if (mTextView != null) {
                ...
                if (mTabSelectedTextSize > 0) {
                    float textSize = selected ? mTabSelectedTextSize : mTabTextSize;
                    mTextView.setTextSize(TypedValue.COMPLEX_UNIT_PX, textSize);
                }

                if (mTabBold == BOLD_SELECTED) {
                    Typeface bold = Typeface.defaultFromStyle(selected ? Typeface.BOLD : Typeface.NORMAL);
                    mTextView.setTypeface(bold);
                }
            }
            if (mIconView != null) {
        ...
        public void onMeasure(final int origWidthMeasureSpec, final int origHeightMeasureSpec) {
            ...
            if (mTextView != null) {
                final Resources res = getResources();
                float textSize = mTextView.isSelected() && mTabSelectedTextSize > 0 ? mTabSelectedTextSize : mTabTextSize;
    ...
    private class SlidingTabStrip extends LinearLayout {
        ...
        private int mSplitterPadding;
        private int mSelectedIndicatorWidth;
        private int mSelectedIndicatorHeight;
        private int mIndicatorVisibility; // tab下的引导线显示状态
        private final Paint mSelectedIndicatorPaint;
        private Paint mSplitterPaint;
        ...
        void setSplitterColor(int splitterColor) {
            mSplitterColor = splitterColor;
        }

        void setSplitterWidth(int splitterWidth) {
            if (splitterWidth > 0) {
                mSplitterWidth = splitterWidth;
                if (mSplitterPaint == null) {
                    mSplitterPaint = new Paint();
                    mSplitterPaint.setAntiAlias(true);
                }
                mSplitterPaint.setStrokeWidth(mSplitterWidth);
            }
        }

        void setSplitterPadding(int splitterPadding) {
            mSplitterPadding = splitterPadding;
        }
        ...
        public void draw(Canvas canvas) {
            ...
            if (mIndicatorVisibility == VISIBLE && mIndicatorLeft >= 0 && mIndicatorRight > mIndicatorLeft) {
                if (mSelectedIndicatorWidth == 0) {
                    ...
                    RectF oval3 = new RectF(tabCenter - mSelectedIndicatorWidth / 2, getHeight() - mSelectedIndicatorHeight,
                            tabCenter + mSelectedIndicatorWidth / 2, getHeight());
                    canvas.drawRoundRect(oval3, 30, 30, mSelectedIndicatorPaint);
                }
            }

            if (mSplitterWidth > 0 && mSplitterPaint != null) {
                final int childCount = getChildCount();
                // tab超过1个是添加tab分割器
                if (childCount > 1) {
                    if (mSplitterColor == 0) {
                        mSplitterColor = Color.parseColor("#CDCDCE");
                    }
                    mSplitterPaint.setColor(mSplitterColor);
                    final int height = getHeight();
                    for (int i = 0; i < childCount - 1; i++) {
                        final int right = getChildAt(i).getRight();
                        canvas.drawLine(right, mSplitterPadding, right, height - mSplitterPadding, mSplitterPaint);
                    }
                }
            }
